...
<div class="outer">
	<div class="settings-top">
		...
		<div
			on:click={() => {
				if (!state.validHard) {
					toaster.pop("Mchezo tayari umekiuka mod ngumu");
				}
			}}
		>
		...
		<Setting type="dropdown" bind:value={$mode} options={modeData.modes.map((e) => e.name)}>
			<svelte:fragment slot="title">Hali ya Mchezo</svelte:fragment>
			<svelte:fragment slot="desc">
			Hali ya mchezo huamua ni mara ngapi neno huonyeshwa upya
			</svelte:fragment>
		</Setting>
		<!-- svelte-ignore a11y-click-events-have-key-events -->
